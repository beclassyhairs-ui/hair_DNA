...
export default function CoupangCardList({
  ...
  return (
    <section className="space-y-2">
      <p className="text-aux font-bold uppercase tracking-[0.12em] text-ink-2">{heading}</p>
      ...
      {cards.map((c) => (
        ...
        >
          <div className="flex items-start gap-3">
            <span className="mt-0.5 text-h2 leading-none">{c.emoji}</span>
            <div className="min-w-0 flex-1">
              <p className="text-body font-bold text-ink">{c.name}</p>
              <p className="mt-1 text-body font-medium leading-relaxed text-ink-2">{c.reason}</p>
              {/* 이유 2~3줄 — 한 줄 요약보다 한 톤 연하게(ink-3)·줄간격 넉넉·펼쳐둠. */}
              {c.detail && (
                <p className="mt-1.5 text-aux leading-[1.7] text-ink-3">{c.detail}</p>
              )}
              {/* 검은 버튼 제거 → 우하단 가벼운 포인트컬러 텍스트 링크. 터치는 카드 전체. */}
              <span className="mt-2 flex items-center justify-end gap-1 text-aux font-semibold text-[#B5542C]">
      ...
      <p className="px-1 pt-1 text-aux leading-relaxed text-ink-2">{COUPANG_DISCLOSURE}</p>
    </section>
  );
}
