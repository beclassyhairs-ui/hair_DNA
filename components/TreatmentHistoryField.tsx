    ...
    <section className={`rounded-card border border-line bg-card px-4 py-4 ${className}`}>
      ...
      <div className="mt-3 flex items-center gap-2">
        <input
          ...
          placeholder="예: 뿌리펌"
          aria-label="시술 이력 입력"
          className="min-h-12 min-w-0 flex-1 rounded-btn border border-line bg-bg px-3.5 text-body text-ink outline-none transition-colors focus:border-ink"
        />
        <button
          type="button"
          onClick={handleAdd}
          disabled={!draft.trim()}
          className="min-h-12 shrink-0 px-3 text-body font-bold text-ink transition-opacity disabled:opacity-40"
        >
          추가
        </button>
      </div>
