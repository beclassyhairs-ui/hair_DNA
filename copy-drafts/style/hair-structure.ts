...
const hairStructure: StyleCopyBlockModule = {
  ...
  entries: [
    ...
    // ════════════════════════════════════════════════════════════════════════
    // 모질 매트릭스 — 굵기(q7) × 숱(q8) 9칸 + 곱슬(q3) modifier.
    //   2026-08-21 사장님 일괄 승인·라이브. 갈래(b3/b6/b10)가 못 덮는 6칸까지
    //   전 손님의 hair-structure 블록을 채운다(과거: 갈래 손님만 aha-ref 1줄, 나머진 빈칸).
    //   틀: "숱은 양이고, 버티는 힘은 한 올의 굵기"(b3_aha 논리)를 9칸에 일반화.
    //   🟡 §0 미결정(실물 보고 정함): 갈래 손님(fine×많음=b3/b10, coarse×많음=b6,
    //      fine×적음=b4/b5)은 aha-ref와 매트릭스 칸이 한 화면에 겹쳐 라이브에 둘 다 나간다.
    //      → aha-ref 유지 vs 매트릭스 통일. 현행대로 배포하고 사업주 결정 대기.
    // ════════════════════════════════════════════════════════════════════════
    {
      ...
      status: "approved",
      sourceGrade: "파생",
      sourceRef: "매트릭스 굵음×많음 — BRANCH_COPY.b6.aha(숱+굵기=옆으로 뜸) 논리 확장",
      evidenceKeys: ["q7_thickness", "q8_density"],
    },
    {
      id: "style.hair_structure.m_coarse_med",
      text: "굵은 머리칼에 숱은 보통이라 실제보다 숱이 많아 보이는 머리예요. 힘이 있고 잘 눌리지 않아서 웬만한 스타일은 다 소화합니다. 굵은 결이 살아 있으니 그 장점을 살리는 쪽으로 가면 됩니다.",
      status: "approved",
      sourceGrade: "재배치",
      sourceRef: "사장님 채굴 답변 2026-08-21 · 2026-09-10 역할표 견본 간결화 — 굵음×보통(성질+장점)",
      evidenceKeys: ["q7_thickness", "q8_density"],
    },
    {
      ...
      status: "approved",
      sourceGrade: "재배치",
      sourceRef: "사장님 채굴 답변 2026-08-21 — 굵음×적음: 숱 많아 보이게·볼륨 살려주는 경우 많음(가능성 톤)",
      evidenceKeys: ["q7_thickness", "q8_density"],
    },
    {
      ...
      status: "approved",
      sourceGrade: "재배치",
      sourceRef: "사장님 채굴 답변 §3-1(숱=양감/질감) — 보통×많음: 숱이 곧 볼륨, 너무 많을 때만 속 양감 조절(덮는 머리는 안 침)",
      evidenceKeys: ["q7_thickness", "q8_density"],
    },
    {
      ...
      status: "approved",
      sourceGrade: "재배치",
      sourceRef: "사장님 채굴 답변 2026-08-21 — 보통×보통: 선택지가 많다(가능성 톤)",
      evidenceKeys: ["q7_thickness", "q8_density"],
    },
    {
      ...
      status: "approved",
      sourceGrade: "재배치",
      sourceRef: "사장님 채굴 답변 2026-08-21 — 보통×적음: 볼륨 많이 살려줘야·다른 시술 보완·제품으로 채움(가능성 톤)",
      evidenceKeys: ["q7_thickness", "q8_density"],
    },
    {
      ...
      status: "approved",
      sourceGrade: "파생",
      sourceRef: "매트릭스 얇음×많음 — BRANCH_COPY.b3/b10.aha(양은 많은데 기둥 없음) 논리 확장",
      evidenceKeys: ["q7_thickness", "q8_density"],
    },
    {
      ...
      status: "approved",
      sourceGrade: "재배치",
      sourceRef: "사장님 채굴 답변 2026-08-21 — 얇음×보통: 보통×적음과 동일 방향(볼륨 보완, 가능성 톤)",
      evidenceKeys: ["q7_thickness", "q8_density"],
    },
    {
      ...
      status: "approved",
      sourceGrade: "파생",
      sourceRef: "매트릭스 얇음×적음 — BRANCH_COPY.b4/b5.aha(통째로 가라앉음, 길이=무게) 논리 확장",
      evidenceKeys: ["q7_thickness", "q8_density"],
    },
