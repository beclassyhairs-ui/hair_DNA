...
// 기장별 커트 확장 — 기장 축 + 레이어·굵기·숱을 본문에 녹임.
...
const cut: StyleCopyBlockModule = {
  ...
  entries: [
    ...
    // ════════════════════════════════════════════════════════════════════════
    // 기장별 커트 확장 — 기장(q11)을 축으로, 레이어(q14)·굵기(q7)·숱(q8)을
    //   본문에 녹인 커트 조언 + 미용실 주문 멘트. 2026-08-21 승인·라이브. 갈래 커트
    //   (b4/b6/b10)가 특정 모질만 덮던 것을, 전 기장 손님에게 기장 맞춤으로 확장한다.
    //   ⚠️ 레이어·굵기·숱을 별도 분기가 아니라 문장 안에서 다룬다(기장 5칸만 도달 축) —
    //      완전한 4축 조합 커버가 아니므로 리뷰 파일에 "미커버(문장 내 처리)"로 명시.
    //   🟡 §0 미결정: b4(롱)·b6/b10(단발 겉결) 손님은 승인 커트 entry와 겹칠 수 있음(실물 보고 정함).
    // ════════════════════════════════════════════════════════════════════════
    {
      ...
      status: "approved",
      sourceGrade: "재배치",
      sourceRef: "사장님 채굴 답변 2026-08-21(2차) — 숏: 두상 1축(굴곡 따라 볼륨·튀나온곳 덜고 꺼진곳 남김·옆통수 돌출), 모질 2축",
    ...
    },
    {
      ...
      status: "approved",
      sourceGrade: "재배치",
      sourceRef: "사장님 채굴 답변 2026-08-21(2차) — 숏보브: 숏과 동일 원리(두상 1축·목덜미 무게·옆통수)",
      evidenceKeys: [...LEN_KEYS],
    },
    {
      ...
      status: "approved",
      sourceGrade: "파생",
      sourceRef: "기장별 커트 — 단발. BRANCH_COPY.b6/b10(겉결 두고 안쪽 양감) 논리를 기장 축으로 확장",
      evidenceKeys: [...LEN_KEYS],
    },
    {
      id: "style.cut.len_collarbone",
      text: "쇄골 길이는 어깨에 닿으면서 바깥으로 뻗치는 기장이에요. 머릿결 문제가 아니라 이 기장이면 다 그렇습니다. 뻗치는 게 싫으면 어깨 아래로 더 기르거나 어깨에 안 닿게 조금 짧게, 살리고 싶으면 그 흐름을 컬로 잡으세요.",
      status: "approved",
      sourceGrade: "재배치",
      sourceRef: "사장님 채굴 답변 2026-08-21(2차) · 2026-09-10 역할표 견본 간결화 — 쇄골: 뻗침 인정+선택지 3",
      evidenceKeys: [...LEN_KEYS],
    },
    {
      ...
      status: "approved",
      sourceGrade: "파생",
      sourceRef: "기장별 커트 — 롱. BRANCH_COPY.b4.procedure(덮는 머리 층, 볼륨 부분 짧게) 논리를 기장 축으로 확장",
      evidenceKeys: [...LEN_KEYS],
    },

    // ── §4 2단 미용실 주문 멘트 — 숏 계열(두상). 겉(_say)+더보기(_why) ──
    {
      id: "style.cut.order_short_say",
      text: "두상 형태 살려서 볼륨 예쁘게 살 수 있게 해주세요",
      status: "retired",
      sourceGrade: "파생",
      sourceRef: "§4 2단 겉(주문 멘트) — 숏 계열 두상. 사장님이 준 주문 문장",
    ...
    },
    {
      ...
      status: "retired",
      sourceGrade: "파생",
      sourceRef: "§4 2단 더보기(이유) — 숏 계열 두상 무게 배분",
      evidenceKeys: ["q11_length"],
    },
  ],
};
