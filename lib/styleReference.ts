...
export function buildHairStylePrompt(answers: StyleAnswers): string {
  ...
  return [
    ...
    `${length}, ${layer}, ${wave}.`,
    ``,
    `=== HAIR COLOR — ABSOLUTE LOCK ===`,
    `Copy the exact hair color from the source image. Do NOT invent or change it.`,
    `If source hair is black or dark brown → output must be black or dark brown.`,
    `If source hair is gray or white → output must be gray or white (do not darken).`,
    `Do NOT add gray, white, silver, or highlighted strands that are not in the source photo.`,
    `Inventing any hair color not present in the source is a FAILED output.`,
    ``,
