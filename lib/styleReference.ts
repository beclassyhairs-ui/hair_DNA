...
export function buildHairStylePrompt(answers: StyleAnswers): string {
  ...
  return [
    ...
    ``,
    `=== HAIR COLOR — ABSOLUTE LOCK (HIGHEST PRIORITY — READ THIS FIRST) ===`,
    `The hair color in the output MUST be 100% identical to the source image. This overrides everything else.`,
    `If source hair is black or dark brown → output MUST be black or dark brown. No exceptions.`,
    `If source hair is gray or white → output must be gray or white (do not darken).`,
    `Do NOT generate gray, white, silver, or highlighted strands not present in the source photo.`,
    `Inventing any hair color not in the source = HARD FAILURE. The color lock cannot be overridden by style descriptions.`,
    ``,
    `=== HAIRSTYLE TO APPLY ===`,
