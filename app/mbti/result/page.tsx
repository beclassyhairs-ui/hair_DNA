  ...
  return (
    <main className="min-h-screen bg-charcoal text-cream">
      ...
      <div className="mx-auto w-full max-w-lg px-5 pb-44">
        <motion.div variants={STAGGER} initial="hidden" animate="show" className="space-y-4 pt-6">
          ...
          <motion.div variants={FADE_UP} className="overflow-hidden rounded-3xl">
            ...
            <div
              ...
              style={{ background: `linear-gradient(to bottom, ${data.gradB}55, rgba(28,26,24,0.95))` }}
            >
              <p className="text-xs text-cream/50">아래로 스크롤해 성향 분석 보기</p>
              <svg
                className="h-4 w-4 animate-bounce text-cream/40"
          ...
          <motion.div
            variants={FADE_UP}
            className="overflow-hidden rounded-2xl border bg-white/[0.04] p-6"
            style={{ borderColor: `${data.gradA}40` }}
          >
          ...
          {/* 공유 + 다른 유형 구경 — 같은 영역에 묶어서 배치 */}
          <motion.div variants={FADE_UP} className="space-y-3">
            {/* 공유 버튼 */}
            <div className="rounded-2xl border border-white/[0.09] bg-white/[0.04] px-5 py-4">
              <p className="text-sm font-semibold text-cream/80">친구는 어떤 유형일까요? 🤔</p>
              <p className="mt-0.5 text-xs text-cream/40">공유하고 서로 비교해 보세요</p>
              <button
                onClick={handleShare}
                className="mt-3 w-full rounded-xl border border-white/15 py-3 text-sm font-semibold text-cream/70 transition-all hover:border-white/30 hover:text-cream active:scale-[0.98]"
              >
                🔗 결과 링크 공유하기
              </button>
            </div>

            {/* 다른 유형 구경하러 가기 — 대형 그라데이션 카드 버튼 */}
            <button
              onClick={() => setShowBrowse(true)}
              className="group relative w-full overflow-hidden rounded-2xl px-6 py-5 text-left transition-all active:scale-[0.98] hover:brightness-110"
              style={{ background: `linear-gradient(135deg, ${data.gradA}80, ${data.gradB}cc)` }}
            >
              {/* 글로우 호버 오버레이 */}
              <span
                className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-300 group-hover:opacity-100"
              ...
              <div className="relative flex items-center gap-4">
                ...
                <span className="flex-none text-lg text-white/50 transition-transform group-hover:translate-x-1">→</span>
              </div>
            </button>
          </motion.div>
        ...
        {/* 쿠팡 파트너스 법적 고지 */}
        <p className="mt-6 pb-2 text-center text-[0.7rem] leading-relaxed text-cream/20">
      ...
      <AnimatePresence>
        {showBrowse && (
          <AllTypesBottomSheet
            ...
            onSelect={(newId) => {
              router.push(`/mbti/result?r=${newId}`);
              setShowBrowse(false);
            }}
          />
        )}
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="relative flex max-h-[82vh] w-full max-w-lg flex-col overflow-hidden rounded-t-3xl border border-white/[0.1] bg-charcoal"
        onClick={(e) => e.stopPropagation()}
      >
        {/* 핸들 + 헤더 */}
        <div className="flex-none border-b border-white/[0.07] px-6 pb-4 pt-4">
          <div className="mx-auto mb-4 h-1 w-10 rounded-full bg-white/20" />
          <h2 className="text-center font-serif text-lg font-bold text-cream">
            다른 헤어 성향 구경하기
          </h2>
          <p className="mt-1 text-center text-xs text-cream/40">
            16가지 유형 중 궁금한 유형을 클릭해 보세요
          </p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4">
          <div className="grid grid-cols-2 gap-2.5">
            {Object.entries(HAIR_TYPES).map(([rawId, type]) => {
              const typeId = Number(rawId);
              const isCurrent = typeId === currentId;
              return (
                <button
                  key={rawId}
                  ...
                  className={`flex flex-col items-center gap-1.5 rounded-2xl border p-4 text-center transition-all active:scale-[0.97] ${
                    isCurrent
                      ? "cursor-default border-white/25 bg-white/10 opacity-60"
                      : "border-white/[0.08] bg-white/[0.04] hover:border-white/20 hover:bg-white/[0.08]"
                  }`}
                  style={!isCurrent ? { borderColor: `${type.gradA}55` } : undefined}
                >
                  <span className="text-2xl">{type.emoji}</span>
                  <span
                    className="font-sans text-base font-black tracking-[0.1em]"
                    style={{ color: isCurrent ? "rgba(255,255,255,0.6)" : type.gradA }}
                  >
                    {type.mbti}
                  </span>
                  <span className="text-xs leading-tight text-cream/60">{type.nickname}</span>
                  {isCurrent && (
                    <span className="mt-0.5 text-[0.6rem] font-semibold text-cream/35">
                      현재 내 유형
                    </span>
                  )}
                </button>
