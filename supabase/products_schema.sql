-- ============================================================================
-- 어뷰티 — 제품 관리(Product CMS) 테이블
-- 빈 데이터베이스 기준 최초 생성 스크립트. Supabase SQL Editor에 전체를 그대로
-- 붙여넣어 한 번에 실행한다 (BEGIN/COMMIT으로 감싸 원자적으로 적용됨 — 중간에
-- 에러가 나면 전체가 자동 ROLLBACK되어 반쪽 상태로 남지 않는다).
-- ============================================================================
...
-- ============================================================================
-- 확장 — 발견템(/items) 노출 파이프라인을 위한 필드. 위 기본 테이블 생성과
-- 같은 트랜잭션에서 이어서 실행된다.
--
...
-- ============================================================================
-- RLS 관련: 이 블록은 RLS 정책을 추가/변경하지 않는다. 위쪽에서 설정한
-- "alter table public.products enable row level security;" 상태와 anon 대상
-- 정책 0개(조회/등록/수정/삭제 전부 차단)가 그대로 유지된다.
