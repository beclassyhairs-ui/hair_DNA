...
## 다음 할 일
1. **④ /style 접수(loading)** — 게이트/킥오프 로직 무변경, 대기 화면 시각만. before/after → tsc0 → 커밋.
2. 이어서 ⑤ 결과지 → ⑥ home/my-hair → ⑦ damage 랜딩·설문 → ⑧ damage 결과지 → ⑨ login/consent → ⑩ items.
   - 게이트 화면(업로드류)은 `_shot_upload.mjs`처럼 /api/auth/me route mock 로 캡처.
