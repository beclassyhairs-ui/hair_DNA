...
- 🟡 **판정 대기 [3-3] /style 플로우**: 데미지와 거의 동일 패턴 — /style 랜딩은 이미 홈 버튼 있음(무수정), 설문/업로드는 단계뒤로만, **/style/result엔 직접 "홈으로" 없음**(저장CTA·처음부터→랜딩만). → **/style/result에도 "홈으로" 추가할지 사장님 판정 대기.** (임의 수정 안 함.)
