...
## 🟢 UI 5단계(노안 배려) 진행 — 5-A·5-B 완료·프리뷰 push, 5-C는 조사만 (2026-08-01)

`feat/ui-ivory-revamp` 프리뷰 브랜치. **프로덕션(main) 미반영.**
...
- **5-C 고민상담소(/consulting) 조사 완료 — 코드 변경 없음, 승인 대기**: **전부 껍데기(프론트 목업)**. 목록=하드코딩 더미(`INITIAL_POSTS`/`HOT_TOPICS`), 글쓰기·답글·좋아요=in-memory `setPosts`만(새로고침 소멸), **인기정렬 로직 없음**(rank 1~4 정적 박힘), **로그인 게이트 없음**(닉네임 하드코딩). import는 useState·AppShell·trackEvent뿐(Supabase/fetch 0). → PM 정책상 "껍데기=톤 정리만, 기능 구현은 카카오 인증과 함께". **톤 정리 착수는 사업주 승인 시.**
...
3. **브랜치 주의**: 프로덕션이 `feat/ui-ivory-revamp`에서 나가고 있고
   `main`은 origin보다 **19 뒤처짐**. 아이보리 리뱀프 3단계 main 머지가
   미결로 남아 있다. **2덩어리(50·60 개편) 정리 시 함께 처리.**
   지금 건드리면 복잡해진다.
