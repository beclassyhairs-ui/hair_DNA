...
## 🔴 런칭 필수 UX(파트1·2·③·④·⑤·⑥) + ddvinh1 세션핑 실측 (2026-08-16, 커밋 완료·**미배포·push 승인 대기**)

사업주 런칭 체크리스트 구현. 프론트 4커밋 + 백엔드 2커밋(Codex 4라운드 통과). 커밋 `d57b4e0`·`4178ef8`·`0f3052c`·`a11dd72`(+ 앞선 파트2·1). **push/배포 미승인.**
...
- 🔴 **배포 전 필수**: ① push 승인 ② **Replicate 잔액 충전**(<$5 → 생성 429로 축소, 유입 전 필수) ③ `public/references` 삭제분·`referencesManifest.json`(Codex 빌드가 삭제분 기준 재생성 → HEAD 복원함) 커밋 금지(각 커밋 선택 스테이징으로 제외 완료).
