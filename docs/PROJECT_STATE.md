...
- **Phase 2 내 정보 삭제 (`bfb5379`)**: /my-diary 하단 '내 정보 삭제'(로그인 손님만·미로그인 숨김) → 확인 모달("진단 기록과 계정 정보가 모두 삭제됩니다. 복구되지 않아요." / 취소·삭제하기) → POST `/api/me/delete`.
  - **삭제 대상 user_id 는 세션 쿠키에서만**(본문/쿼리 무시) → 타인 데이터 삭제 경로 없음. 무세션 401.
  - Supabase RPC **`soft_delete_user`**(단일 트랜잭션): 자식(diagnoses/profiles/hair_usage/events) 실제 DELETE + users soft-delete(kakao_user_id 센티넬 `deleted:`||id · nickname/profile_image null · marketing_consent false) + user_consents 보존(감사). 재삽입 race 는 assert_parent_active 트리거로 차단.
  - 성공/401/이미삭제 전 경로에서 세션·스로틀 쿠키 만료. 완료 후 로컬 `abeauty*` 키 전부 클리어 → /style 이동(전체 리로드).
  - **삭제계정 서명쿠키 방어**: `/api/auth/me` 가 `deleted_at` 확인 → 삭제계정이면 loggedIn:false + 쿠키 만료(조회실패 fail-open). (Codex [높음] 지적 반영.)
...
- **타이포 토큰**(tailwind.config): 본문 17→15 · 보조 15→13 · 소제목/문항 h2 19(유지) · 스탬프 stamp 16(신설) · 칸 라벨 label 12(신설) · 스타일명/제목 h1 24(유지) · 하단탭 nav 11(신설). **굵기 700→600**(제목·버튼·스탬프), 본문 강조 ≤600.
- **globals 토큰**: --line #e9e3d8→#ebe8e0(연하게) · --r-card 16→12 · --r-btn 12→24(필 유지) · --shadow→none(그림자 전역 제거, boxShadow.soft=var(--shadow)) · .btn 높이 54→48·radius 30→24·그림자 제거·weight 600. card-soft 그림자 제거·선 --line(배경색은 기존 rgba .94 유지).
- **하드코딩 px→토큰 치환**(스코프 10화면+공통 컴포넌트): text-[17]→body·[15]→aux·[13]→aux·[14]→aux·[26]→h1·[19]→h2·[12]→label / eyebrow→label · 스탬프→stamp · 하단탭→nav. 터치 min-h-14/h-14→min-h-12/h-12(48). 설문 선택지 카드 min 48. 하단탭 높이 56 유지(min-h-14).
  - 잔여 하드코딩(의도적 유지): 이미지 위 오버레이(Before/After 9~11px)·FadePreview ▼·디버그 패널.
...
- **강제 5060 기본선**: 본문 17px↑ · 보조 15px↑ · 터치 56px↑ · 뒤로가기 좌상단 · 하단 고정 CTA · 진행표시 상시 · 색대비 AA.
- **Phase 1 토큰**(`6c8f2ee`): body 16→17·aux 14→15·h1 22→24·h2 18→19·emphasis 16→17 / `--ink-3` #b7b0a5→#7a7268(AA).
- **Phase 2 화면 ①~⑩**(각 커밋+before/after): ①/style 랜딩(CTA 하단고정) ②설문(선택지 크게·뒤로 좌상단) ③업로드 ④접수 ⑤결과지(본문17·보조15) ⑥/home·/my-diary(카드15→17·전역 하단탭) ⑦damage 랜딩·설문 ⑧damage 결과지 ⑨login/consent(터치56) ⑩items·제품카드.
  - 공용 컴포넌트 restyle 파급: RoundedOptionButton·TestHeader·BottomNav·Button·BlackCTAButton·FadePreview·CoupangCardList·TreatmentHistoryField.
...
> **배포**: 2026-09-10 `157d16c..99b2882` origin/main push → Vercel 자동배포. 배포 커밋 **`99b2882`**(HEAD==origin/main). 로컬 타입게이트(tsc 0·copy:check OK) 통과. Vercel 빌드 Ready·라이브 실합성 3경로는 사장님 폰 확인(로그인 세션 필요).
...
- ✅ **429 판정 반영 (`537cef9`)**: 일일한도(429)는 접수화면 대신 **결과지로** — 진단 본체 노출·사진 칸만 "오늘 사진은 여기까지예요/내일 다시" 안내(폴링 0). 401·403·network는 접수화면 유지. 계측 photo_state에 `limited` 값 추가(pending과 구분). 문구 배치는 현행(사진 칸 아래 전폭 블록) 유지 판정. Codex b관점 재확인 통과(새 우회 없음). tsc 0·14/14·9/9.
...
- **검수 통과 확인(회귀 0)**: [1] 내부 키(`abeauty_*` 스토리지/쿠키/서명 payload) 전건 무손상, 손님 대면 "어뷰티" 잔재 0(masterData 7건 의도보존만) · [2] 로딩 상한5분/트리거4:50, 스킵(10초·job존재)은 `router.push`로 재차감·중복합성 불가 · [3] 홈카드 빈→/style·채워짐→/my-diary, 문의 3곳 `enabled&&href`+`target=_blank rel=noopener` · [4] 가드레일 전건 생존(401·403·429·quota 검증후 선차감·셀카 미저장·SSRF allowlist·결과지 직접URL 차단) · [5] warmup 전역 원자 쿨다운 배선 정상(RPC 오류만 fail-open).
- 🔴 **발견·수정 [폴백 죽은경로]**: Phase2(`be4056e`)가 클라 폴백 트리거를 8분→4:50(290s)으로 당길 때 서버 `FALLBACK_MIN_ELAPSED_MS`(7분=420s)를 안 내려, **정상 콜드미스 폴백이 전건 `fallback_not_eligible`로 거부**되던 회귀(=콜드스타트 첫 손님이 "5분 안에 이미지" 대신 에러). Codex 교차확인 "죽은 경로". 사업주 판정 = 수정.
- ✅ **fix (`46ed39a`, 5파일)**: `FALLBACK_MIN_ELAPSED_MS` 7분→**240초**(클라 290s 트리거보다 낮아 정상 콜드미스 통과 / 즉시cancel `completed−created≈0`·즉시재요청 `now−created≈0`·폴백체이닝 attestation 차단은 유지 — Codex 통과). 클라·서버 상수 **상호의존 경고 주석** 양쪽 배치. 낡은 "8분/7분" 수치 주석 전수 정리(현행=상한5분·트리거4:50·문턱240초, 역사서술 2곳만 보존). **손님 문구 무변경**("평균 2~3분, 최대 5분 걸려요" 유지). loading:104 개발자주석에 best-effort 한계(폴백 예산 2분→극단 ~6:50) 명시.
- ✅ **test (`1dabfb2`, 4파일)**: 판정식만 순수함수 `lib/fallbackEligibility.ts`(`isFallbackElapsed`)로 분리(now 를 함수로 받아 원본과 평가시점까지 동일 — Codex 2회 "동작차 없음") + `FALLBACK_MIN_ELAPSED_MS` 이관. **`npm run test:fallback` 11/11**(4경계·now()평가시점 계약·서버문턱<클라트리거 정합). `test:invariant` 9/9 무회귀. 서버 문턱 회귀는 자동 감시, 클라 트리거는 미러(상호의존 주석 1차 방어).
- **검증법(손님 데이터 무관)**: ①하네스가 결정적 증명(채택). ②프리뷰 poison E2E는 사업주 판정으로 **미채택**(되돌리기 실수 위험) — 실콜드미스는 오픈 후 첫 손님에게 자연발생하니 Replicate 대시보드에서 lucataco 예측 확인.
- ✅ **push·배포 완료 (2026-09-09)**: `157d16c`까지 origin/main → Vercel 자동배포. 로컬 타입게이트(tsc --noEmit·copy:check) PASS, 라이브 서빙 확인.
- ✅ **F-2 표적 재검수 (2026-09-09) — 새 🔴 0**: [1] 배포본 문턱 240s·정합 확인 [2] **하네스 자기검증**(문턱 300s로 훼손→5건 실패로 실제 포착, 원복 후 11/11 = 가짜 안전망 아님) [3] 순수함수 분리 원본과 한 줄 대조 동일 [5] 가드레일 앵커 무손상. **Codex 적대적 검수 2건은 전부 기존 수용 리스크**(내 수정 무관): ① 문턱<트리거 필수 특성상 그 값부터 "선행 우회" 가능(피해=손님 자기 quota로 미화결과, 미미) → **240초 유지 확정**(상향은 방금 고친 정상경로 마진을 다시 위태롭게 함) ② attestation 미소진·quota fail-open → **결제 붙일 때 `hair_jobs` 원장으로 정식 해결**(A 섹션 로드맵, quota 7/일로 완충). 
- 🟡 **오픈 후 로드맵**: attestation 원자 소비(`hair_jobs` 원장) — 결제 연동과 세트. ⚪·🟡 잔여(masterData 7건·미참조 이미지2·debug-sentry·mature PNG·폰트 이원화·설문 이탈·문의 http)는 판정대로 오픈 후.

## 🟡 C-2. 데미지 랜딩 히어로 축소 + 홈 카드 이미지 + 뒤로가기 (2026-09-05, 커밋 완료·미push)

C 라운드 실물 확인 결과 2장 세로배치가 무거워 1장 축소 + 발견된 2건 처리. ★ 설문·진단로직·점수·결과지·CTA·게이지·배지·h1 불가침 / /style 랜딩 무수정.

- ✅ **[1] 히어로 1장 (`8269970`)**: 건강+상한 2장 → 상한 결 1장(damage-hero-damaged.png, 정사각 max-w-260, 라벨 없음). TextureSample 제거. 375px 실측 결 또렷. LandingFrameHero 무수정. healthy 파일은 삭제 안 함(미참조·릴스용).
- ✅ **[2] 홈 카드 (`8c706c2`)**: 홈 '손상도 체크' 카드 썸네일 /landing/damage-hero.jpg → /images/damage-hero-damaged.png. 스타일 카드·규격(4:5 w-20)·배치 무변경. 80px에서도 결 식별.
- ✅ **[3] 홈 복귀 (`cbcb7f8`)**: 데미지 플로우에 BottomNav(3탭) 없음 확인 → 랜딩 좌상단 홈 아이콘 + 결과지 하단 "홈으로 →" 추가. 설문은 미변경(기존 나가기→랜딩→홈 경로 + 새 무확인 이탈은 [3-4] 범위밖). 공용 TestHeader 무수정.
- ✅ **[3-3] /style 결과지 홈 링크**: 판정대로 /style/result 하단(저장/공유/재진단 아래·문의 블록 위)에 데미지와 동일 위치·문구("홈으로 →")·스타일로 추가 → 두 결과지 동선 일치. /style 랜딩(이미 홈 있음)·설문·업로드는 무수정.
- 🟡 **보고만 [3-4]**: 데미지·스타일 설문 모두 중간 이탈 시 입력 소실(sessionStorage 미저장 구간), 이탈 확인 없음. 이번 범위 밖 — 필요 시 별도 라운드에서 확인 다이얼로그.
- 🔴 **다음 = 사업주 승인 후 push·배포.**

## 🟢 E. 문의 창구 오픈 — CONSULT_CHANNEL 켜기 (2026-09-05, push·배포 완료)
...
- ✅ **Phase 0**: 상단 구조·스타일 랜딩 규격 비교·사진 2장 실측(1448×1086 4:3, 각 2.2MB, 배경 회색 vs 베이지) 보고 → 판정 수령(톤=타이트크롭①만·레이아웃=세로2단·1:1정사각·프레임=데미지전용 마크업·EXAMPLE 제외·h1 유지).
- ✅ **구현 (`61187fc`)**: `app/damage-check/page.tsx` 히어로 블록만 교체. 세로 2단(위 건강한 결/아래 상한 결) 한 카드, 1:1 정사각 object-cover 동일 크롭(원본 무편집), 라벨 차콜칩, next/image. 새 이미지 2장(`public/images/damage-hero-{healthy,damaged}.png`) 커밋 포함.
  - ★ 공유 컴포넌트 `LandingFrameHero` 무수정(=/style 불변). 데미지 전용 `TextureSample` 로컬 컴포넌트.
  - **375px 실측**: 세로 2단에서 결(윤기 vs 푸석·잔머리) 또렷이 식별. 불가침 블록 전부 유지.
  - 톤: 타이트 크롭으로 배경 노출 최소화 → 잔여 톤차 경미(한 세트로 읽힘). 공통필터②는 미적용(필요 시 판정 후).
  - Codex 통과. 옛 `public/landing/damage-hero.jpg`는 이제 미참조(잔존·무해).
- 🔴 **다음 = 사업주 승인 후 push·배포.** 확인법: /damage-check 상단에 결 견본 2장(위 건강/아래 상한), 그 아래 배지·h1·CTA 그대로.

## ℹ️ 배포 정정 (2026-09-05): B(홈 '나의 스타일' 카드)·태그라인 노출 정비는 push·배포 완료(`d1aa848`까지 origin/main). 아래 B 섹션의 "미push"는 낡음.
...
지시서 「D. 서비스명 전면 교체」. 서비스명 **"미알팁"**, 태그라인 **"미용실에서 알 수 없는 꿀팁"**. 로마자 **MIALTIP/mialtip**. **손님이 눈으로 읽는 텍스트 + OG 이미지만 교체 — 로직·내부 식별자 일체 무변경.** Phase별 커밋 분리·손님대면 커밋 Codex 검수·push 안 함(배포는 사업주 승인 후).

- ✅ **Phase 0 (읽기전용)**: 119파일 전수조사 → (A)화면텍스트/(B)내부식별자/(C)주석·문서 분류표 + 법적문서·OG·워터마크 현황 보고 → 사장님 판정 수령.
- ✅ **Phase 1 (`36697fd`)**: 손님·어드민 화면 텍스트 28파일 교체. 워드마크 A-BEAUTY→MIALTIP(4랜딩), 헤더/푸터/어드민 표시명, 전 페이지 메타데이터·OG·공유문구, 루트 title/description 태그라인 반영, 다운로드 파일명 `abeauty-`→`mialtip-`, 레거시 /result 본문. **Codex 통과**(식별자 무변경·순수 치환·tsc --noEmit 통과).
- ✅ **Phase 2 (`1a497e8`)**: 법적문서(이용약관·개인정보처리방침) 서비스명만 교체. 사업자 정보(상호·대표자·등록번호·통신판매업 신고번호) 무변경. **Codex 확인**.
- ✅ **Phase C (`a73e51f`)**: 코드 주석 43파일 + 현재형 문서(CLAUDE.md·.env.local.example·PROJECT_MASTER_CODE.md) + lib/brand.ts 값 갱신(import 0건 유지, SSOT 배선 안 함). 로직 0(diff 55줄=주석 53+상수 2). 별도 커밋(독립 롤백용).
- ✅ **Phase 3 (`eb73133`)**: `public/og-default.png` 이름없던 플레이스홀더 → "미알팁 + 태그라인" 카드(아이보리+차콜+골드바, playwright canvas 렌더). 자체 OG 2종(bangs-og·hair-mbti-og)은 옛이름 각인 없어 무변경. OG 메타 그대로(같은 파일명).
- ✅ **★(B) 내부 식별자 무변경 증거**: `abeauty_user_profile`·`abeauty:diaryEntries`·쿠키(`abeauty_session`/`_admin`/`_oauth`/`_seen`)·서명 payload(`abeauty-*-v1`)·User-Agent 전건 그대로. grep 확인 36건 유지, `mialtip` 등장은 다운로드 파일명 1곳뿐. **기존 저장 데이터 안 끊김.**
- 🔴 **다음 = 사업주 승인 후 push·배포.** 배포 전 육안 확인: ① **OG 카드 디자인 승인**([사업주 승인] 대상, 마음에 안 들면 같은 파일명 교체) ② 각 랜딩 상단 MIALTIP 워드마크·헤더 "미알팁" ③ 카톡 공유 미리보기.
- 🟡 **미결(사장님 판단 대기)**: ① **result/masterData.ts 가상 제품명 7건**("어뷰티 뿌리 볼륨 에센스" 등) — 서비스명 아닌 자체 브랜드 제품명이라 "미알팁이 제품 판다" 오해 방지 위해 **의도적 제외**. ② **이용약관 제1조** `미알팁(이하 "회사")` — 기존 구조인데 실제 사업자 상호와 다를 수 있어, 상호 확정 후 "서비스=미알팁 / 회사=사업자 상호" 분리 재정비 권장.
- ℹ️ **정리 필요(삭제 차단됨)**: repo 루트 임시파일 `_og_gen_tmp.html`·`_render_og_tmp.mjs`(OG 렌더용, untracked·미커밋). 환경이 삭제를 막아 사업주가 직접 제거.

## 🟡 A. 로딩 화면 라운드 — Phase 1~3 커밋 완료·미push·미배포 (2026-08-30)

지시서 「A. 로딩 화면」. "5분 안에 무조건 이미지" 약속을 정직하게 제시하고, 기다리기 싫은 손님은 먼저 결과지로 보내되 사진은 뒤에서 계속 받게 한다. **Phase별 커밋 분리·전건 Codex 검수 통과·push 안 함(배포는 사업주 승인 후).** 건드리지 말라는 로직(세션핑·폴백 자격검증/킬스위치·서명·quota·가드레일)은 무수정.

- ✅ **Phase 1 (`1b934b4`)**: 대기 상한 `POLL_BUDGET_MS` 480_000→**300_000(5분)**. 진행감(N분 N초째·4단계 순차공개) 위에 주문구 **"평균 2~3분, 최대 5분 걸려요"** 크게 배치("최대 N분"은 `POLL_BUDGET_MIN` 파생이라 상한과 안 갈라짐). `loading/page.tsx`.
- ✅ **Phase 2 (`be4056e`)**: 루카타코 폴백 트리거를 8분 소진 후 → **4:50(290초, `FALLBACK_TRIGGER_MS`)로 당김.** 5:00 상한 직전에 상시-warm 폴백이 약속을 지킴. 폴백 로직 무수정, 발사 '시점'만 당김. 5:00 전 발사 보장 3중 방어(폴 타임아웃 clamp·트리거 도달 시 즉시 break·cancel fire-and-forget). fellBack 가드로 폴백 poll 재트리거 방지.
- ✅ **Phase 3 (`4351643`)**: 로딩 **10초 후 밑줄 텍스트 스킵링크**(job 실존 시에만 노출) → `finishAndRoute` 안 거치고 `STYLE_JOB_KEY` 유지한 채 `router.push`로 결과지 이동. 백그라운드 폴링(cleanup 없는 useEffect라 언마운트돼도 지속)이 사진을 만들어 `STYLE_GENERATED_KEY`에 쓴다. 결과지는 **`pending`("준비 중" 슬롯) + `STYLE_GENERATED_KEY` 2초 감시**로 완성 시 부드럽게 사진 교체. **결과지가 status API를 직접 폴링하지 않고 sessionStorage만 감시** → 폴백 트리거·자격검증은 로딩 백그라운드 폴링이 담당(그 로직 무수정). 정상·폴백 경로 모두 같은 키로 도착. watcher 6분 타임아웃(try 밖 선검사). 과약속 금지 문구.
- 🔴 **다음 = 사업주 승인 후 push·배포.** 검증: 결과지 준비중→자동채움·정상플로우(job 없음) 회귀 없음 dev 실측 통과. **한계(과제)**: 탭 닫으면 백그라운드 폴링 끊김(hair_jobs 서버 원장 없이는 백그라운드 완성 불가) — 결제 붙일 때. ⚠️ 폴백 '최대 5분'은 lucataco 상시-warm 전제의 best-effort(폴백 자체 예산 2분은 무수정이라 lucataco도 느린 극단 케이스는 초과 가능).
...
- ✅ **스타일 Phase 2 — 승인·배포 완료 (2026-08-21, 라이브)**: 사장님 채굴 원문 기준 29건 작성 → **사장님 일괄 승인 → `approved` → 배포.** `[PHASE2]` 게이트 예외 마커 제거·정식 게이트 편입(draft 0·approved 171). `copy:check`(도달 168·죽은칸 0·스모크 7/7·firm 누출 0)·`test:invariant` 9/9(엔진 무변경) 통과. **이제 스타일 결과지 「내 모발 구조」·「궁합」·「커트」가 전 손님에게 채워져 나간다**(과거 대부분 빈칸).
  - 🟡 **§0 미결정 2건(실물 보고 정함)**: ① 매트릭스 ↔ 기존 aha 중복(숱많음 손님은 라이브에 둘 다 노출 — 통일 여부) ② 커트 §4 2단을 단발·롱에도 확장할지. 현행대로 배포됨.
  - **작성분(29건, ★채굴 종료)**: 모질 매트릭스 굵기×숱 9칸 + 곱슬 modifier 3(`hair-structure`) · 궁합 디자인 차등 6(바퀴 수) + §4 2단 주문멘트 4(`curl-fit`) · 커트 기장 5 + §4 2단 주문멘트 2(`cut`). resolver가 원답(q7×q8×q3×q13×q11)을 직접 읽어 배선(판정 엔진 무수정). 분류 = 재배치 18·파생 11·신규 0.
  - **1차+2차 채굴 반영**: 커트 숏·숏보브(두상 1축+모질 2축)·쇄골(뻗침 인정+선택지 3)·곱슬 3종(반곱슬 유리·곱슬 숱교차·악성 매직전제)까지 사장님 실무 답변으로 전면 재작성. 옛 AI 초안(시작점/롯드굵기·층 시작점·설문멘트 재활용) 폐기.
  - **배선 방식**: `[PHASE2]` 마커 draft를 실 registry·resolver에 넣어 dev 실화면 렌더 + `check.ts` 게이트 예외(승인 시 마커 제거→정상 편입). 리뷰 파일 `V2_NEW_COPY_REVIEW_PHASE2.md`.
  - **§4 2단 구조 신설**: 궁합·커트 블록에 겉[미용실 주문 멘트] + 더보기[원장 이유 FadePreview]. `app/style/result/page.tsx` 렌더 반영(draft 없으면 아무것도 안 나감=회귀 0).
  - 🔴 **다음 = 승인만 남음**: 사장님 빨간펜(리뷰 §0 결정 — aha 중복 통일 여부·커트 2단 단발/롱 확장 여부) → `draft`→`approved` + `[PHASE2]` 마커 제거 일괄 반영 → 배포. **추가 채굴 없음(종료).**
  - ✅ **데미지 건조(slow) 사장님 원문 보강** 완료(`damage/drying`, 2원인 분기, 파생, approved) — 커밋 예정.
...
## 🟩 단백질 2종 분리 (2026-08-21 사업주 확정 · 문헌 확인 · 어떤 세션도 어기지 말 것)

> "단백질 폭탄"이라 뭉뚱그려 온 것의 정체가 규명됨. **이름만 같고 완전히 다른 두 물건**이다. 확정54·55(단백질 금지)와 스타일 결과지(가는 모발에 단백질 권장)의 충돌을 이 규정이 해소한다.

1. **① 펌 전처리 단백질 = 좋은 것** — PPT·LPP·단백질 앰플. 중분자 가수분해 케라틴(~2,577 Da)이 모발 **내부(cortex)에 침투해 심지를 보강**. 탄성↑·끊김↓. **스타일 결과지에서 가는 모발에 권하는 단백질은 이것.**
2. **② 케라틴 클리닉 = 경계 대상** — 신데렐라 클리닉·브라질리언 케라틴. 가수분해 케라틴 **+ 알데하이드계**(포름알데히드·글리옥실산 등)를 매직기 **200~232°C 고열**로 표면 가교. 감을수록 풀리는 semi-permanent. 글리옥실산 단백질 손실 3.5 μg/g(처녀모 1.12).
3. **분자량이 결과를 가름**(Malinauskyte 2021): 저분자 221 Da=침투하나 강도 개선 없음 / **중분자 ~2,577 Da=침투+강도↑(정답)** / 고분자 75,440 Da=표면 흡착만.
4. **코드 반영 규칙(불변)**:
   - **확정54·55의 "단백질 금지"는 ②(케라틴 클리닉 반복 이력)에만 적용.** 펌 전처리 ①과 무관 → 스타일의 단백질 권장은 데미지 랜딩 케라틴 경고와 **충돌 아님.**
   - 손님 카피에 **성분명·수치·문헌 용어 노출 금지.** "속을 채운다"는 **체감 언어로만.** 광고 금지어(복구·영양) 회피(확정106) 유지.
5. **출처**: Malinauskyte et al., *Int. J. Cosmetic Science* 2021 / PMC10075350(산·알칼리 스트레이트닝 모발 섬유 영향) / Dermatology Times(Keratin Treatments and Formaldehyde).
...
- **(5) Q3 건조 시간기반 3단계 개편(2026-08-21 안건 등록)**: 현행 Q3(slow/normal/fast 주관 3값)를 **자연건조 실측 시간 3단계**(5분 안 / 5~10분 / 15분 이상)로 개편. **15분 이상은 두 원인으로 분기** — ① 숱 많음(물리적, 중립) ② 단백질 유실→흡수성(손상). 근거 = 사장님 원문 2026-08-21("오래 마르는 건 두 가지 — 숱이 많아 물리적으로 오래 vs 단백질 구성 유실로 흡수성↑"). **이번에 카피(`damage.drying.slow`)는 두 원인 분기를 이미 반영했으나(`파생`, 라이브), 엔진은 여전히 slow=+1.0 단일 가산이라 카피↔엔진 정합이 아직 어긋나 있다** — 이 (5)에서 시간 3단계 + 원인 분기를 엔진·설문에 함께 넣어 정합을 맞춘다. **중복체크·길이문항(위 1·2)과 한 번에 일괄 진행** — 설문 스키마를 한 번만 건드리기 위함. ⚠️ 결과지 V2 진행 중에는 손대지 않는다(스키마 불변 원칙).
